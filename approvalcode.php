...
define("APPROVALCODE_VERSION", "1.6.13");


/**
 * Approval Code Transaction Class
 *
 */
class umTransaction {
	...
	function umTransaction()
	{
		...
		$this->software="Approval Code PHP API v" . APPROVALCODE_VERSION;
	...
	function Process()
	{
		...
		// Figure out URL
		$url = ($this->gatewayurl?$this->gatewayurl:"https://secure.approvalcode.com/gate");
		if($this->usesandbox) $url = "https://sandbox.approvalcode.com/gate";
	...
	function ProcessQuickSale()
	{
		...
		// Figure out URL
		$url = ($this->gatewayurl?$this->gatewayurl:"https://secure.approvalcode.com/soap/gate/C9B0D35F/"); // C9B0D35F/usaepay.wsdl
		if($this->usesandbox) $url = "https://sandbox.approvalcode.com/soap/gate/C9B0D35F";
	...
	function getSession()
	{
		...
		// Figure out URL
		$url = 'https://secure.approvalcode.com/interface/profiler/';
				...
				if(!$ch) {
					...
					if($return["UMversion"]) {
						...
						if($return["UMversion"]) {
							...
							$ch = curl_init("https://209.239.233.73/secure/gate.php?VersionCheck=1&UMsoftware=" . rawurlencode($this->software));
							curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
							curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, 0);
							curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, 0);
							$result=curl_exec($ch);
